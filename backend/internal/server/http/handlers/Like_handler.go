...
func LikeHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		var like map[string]string

		logger.InfoLogger.Println("POST like")
		...
		userID, err = strconv.Atoi(like["user_id"])
		if err != nil {
		...
		likeID, err = strconv.Atoi(like["post_id"])
		if err != nil {
		...
		if len(id) != 0 {
			...
			_, err = db.Exec("INSERT INTO like(user_id, post_id)"+
	...
	case http.MethodGet:
		...
		if len(id) != 0 {
			logger.InfoLogger.Printf("GET: likes for post with id %s\n", id)
			...
			if len(data) == 0 {
				logger.InfoLogger.Println("There are 0 likes for this post")
			}
